...
def step_train(dataloader, model, criterion, optimizer, writer, epoch, **kwargs):
    ...
    for batch in dataloader:
        ...
        rec, rec_k = model(img_u, k_u, mask)
        ref_k = i2k(img_gnd)
        loss = criterion(rec_k * (1 - mask), ref_k * (1 - mask)) / torch.count_nonzero(1 - mask)
...
def step_test(dataloader, model, criterion, work_dir, writer, epoch, **kwargs):
    ...
    for batch in dataloader:
        with torch.no_grad():
            ...
            if torch.cuda.is_available():
                img_u, k_u, mask, img_gnd = img_u.cuda(), k_u.cuda(), mask.cuda(), img_gnd.cuda()
            pred, rec_k = model(img_u, k_u, mask)
            ref_k = i2k(img_gnd)
            loss = criterion(rec_k * (1 - mask), ref_k * (1 - mask)) / torch.count_nonzero(1 - mask)
...
parser.add_argument('--data_path', type=str, default='/root/LITT_data_128')
parser.add_argument('--mask_path', type=str, default='/root/LITT_mask_128_8x_c4')
...
parser.add_argument('--sampled_lines', type=int, default=4)
...
parser.add_argument('--work_dir', type=str, default='/root/log/128_cs_8x_c4_cos_CRNN_idc_mask_reduction')
...
train_transform = data_aug(block_size=(128, 64), rotation_xy=False, flip_t=False, cut_edge=(0, 16))
...
rec_net = CRNN_IDC(uni_direction=args.uni_direction)
