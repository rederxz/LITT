...
from model.model_zy import RRN_two_stage_only_hidden_relay
...
def step_train(dataloader, model, criterion, optimizer, writer, epoch, **kwargs):
    ...
    for batch in dataloader:
        ...
        for i in range(img_u.shape[-1]):  # iterate over t dimension
            output_o, output_h = model(img_u[..., i], k_u[..., i], mask[..., i], output_h)
...
parser.add_argument('--work_dir', type=str, default='/root/log/75')
...
rec_net = RRN_two_stage_only_hidden_relay(n_blocks=3)
