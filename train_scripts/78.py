...
from utils import from_tensor_format, mag_min_max_normalize


def late_normalization(img_gnd, img_u, k_u, mask):
    img_gnd = mag_min_max_normalize(img_gnd)
    img_u = mag_min_max_normalize(img_u)
    k_u = np.fft.fft2(img_u, norm='ortho')
    return img_gnd, img_u, k_u, mask
...
train_dataset = LITT_v3(img_dir=args.data_path,
                        ...
                        after_transform=late_normalization)
