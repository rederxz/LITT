...
from data import LITT_from_np_array_se
...
parser.add_argument('--mat_path', type=str, default='/root/autodl-nas/test_data/test_data_31/res_106_single.mat')
...
with h5py.File(args.mat_path, 'r') as mat_file:
    ...
    k = np.array(mat_file['kspace_filled']).transpose()
    k = k['real'] + k['imag'] * 1j
    mask = np.array(mat_file['mask_filled']).transpose()
...
# inference
_, _, z_total, coil_total, _ = k.shape
...
for z in range(z_total):
    ...
    for coil in range(coil_total):
        ...
        test_dataset = LITT_from_np_array_se(k[:, :, z, coil],
                                             mask[:, :, z, coil],
                                             nt_network=1)
